...
struct RigidBody
{
	...
	RigidBody()
		:position(0.0f), mass(1.0f), force(0.0005f),color(0.1f,0.5f,0.3f,1.0f) {
	}
};
...
void RigidBodyGenerator::init()
{
	...
	this->I_cube = glm::make_mat3(Icube);
...
void RigidBodyGenerator::Update(GLfloat dt)
{
	...
	*/
	...
	//glm::mat3 I = R * I_cube * glm::transpose(R);
	...
	//ortho normalization
	this->rigidbody.orientation = glm::orthonormalize(this->rigidbody.orientation);
	...
	for (int i = 0; i < model.meshes.size(); i++)
	{
		for (int j = 0; j < model.meshes[i].vertices.size(); j++)
		{
			...
			Vertex &vR = mesh[i].vertices[j];
			vertex.Position =this->rigidbody.position +(this->rigidbody.orientation*vR.Position);
			vertex.Normal = this->rigidbody.orientation*vR.Normal;
		}
	...
	for (int i = 0; i < model.meshes.size(); i++)
	{
		for (int j = 0; j < model.meshes[i].vertices.size(); j++)
		{
			...
			if (j == 2 && !forceStop) {
				glm::distance(this->rigidbody.position, vertex.Position);
				forceStop = true;
				//cout << "pos" << endl;
				//cout << glm::to_string(this->rigidbody.position) << endl;
				//cout << "vertex" << endl;
				//cout << glm::to_string(vertex.Position)<< endl;
				this->rigidbody.torque += (vertex.Position - this->rigidbody.position) * glm::vec3(0.001f);
				//cout << "torque" << endl;
